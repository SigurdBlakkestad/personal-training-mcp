...
> **Heads-up on Strava — an active subscription is now required.** As of mid-2026 Strava moved API access behind a paid subscription and deactivates apps that don't hold one. A deactivated app returns `403 Forbidden` (`Application / Status / Inactive`) on every activity call *no matter how valid your tokens are* — re-authenticating does not help. **To sync Strava you must keep an active Strava subscription** and authorize your API app with the `activity:read_all` scope. Without a subscription, leave `sync_strava` disabled; Garmin covers your activities.
>
> **Heads-up on Garmin.** The Garmin sync relies on [`python-garminconnect`](https://github.com/cyberjunky/python-garminconnect), an *unofficial* reverse-engineered API wrapper. Garmin can change endpoints or rate-limit logins at any time, so this collector may break without warning. When it does, `sync_garmin.yml` now **fails loud (red run)** rather than passing silently — recovery is to re-run `scripts/garmin_auth.py` locally and update the `GARMINTOKENS_B64` secret. Because each source is its own workflow, a red Garmin run never blocks Strava, Withings, or the derived metrics.
>
> **What this means for your sources.** Garmin is now the primary source for your activities (rides/runs recorded on a Garmin device) *and* for device metrics (training load, HRV, sleep detail). Withings (body composition, sleep) uses an official, stable API and is unaffected. Strava is optional: only worth wiring up if you keep a Strava subscription, in which case it acts as a second, official activity source — Garmin still takes priority for device-measured fields.
